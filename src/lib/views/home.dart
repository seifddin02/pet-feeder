...
class _HomePageState extends State<HomePage> {
  final String serverUrl =
      "http://192.168.0.100:8000"; // Replace with your Python server IP
  ...
  Future<void> _dispenseFood() async {
    try {
      final response = await http.post(
        Uri.parse('$serverUrl/dispense_food'),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        },
        body: jsonEncode({'target': 1500}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        _showSnackBar('Error dispensing food: $errorMessage');
      }
    } catch (e) {
  ...
  Future<void> _activateBuzzer() async {
    try {
      final response = await http.post(
        Uri.parse('$serverUrl/activate_buzzer'),
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        },
        body: jsonEncode({'delay_time': 3000}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        final errorMessage =
            jsonDecode(response.body)['detail'] ?? 'Unknown error';
        _showSnackBar('Error activating buzzer: $errorMessage');
      }
    } catch (e) {
      _showSnackBar('Connection error: $e');
    }
